...
class LoginAsTeacher extends React.Component {
    ...
    handleLoginClick = () => {
        ...
        axios.post(apis.teacherLogin, qs.stringify(params))
        .then((response) => {
            if(response.data.message) {
                if(response.data.message === 'teacher not found'){
                    NotificationManager.error('Please register', 'Teacher not found', 3000);
                    return;
    ...
    render() {
        ...
        return(
            <div className="mt-5 pt-1">
                <div className="mt-5">
                    <p className="text-light" style={{fontSize: 35, textAlign: 'center'}}>Teacher Login</p>
                </div>
                <div className="mb-5">
